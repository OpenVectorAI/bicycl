           ...
           const string &pre)
{
  ...
  for (size_t iter = 0; iter < niter; iter++)
  {
    ...
    try
    {
      ...
      {
        data1.emplace (data1.end(), Keygen1 (C, n, t, i));
      }
      ...
      for (unsigned int i = 0; i < n; i++)
      {
        data2.emplace (data2.end(), Keygen2 (C, data1[i], randgen, CoQ,
                                                Q_vec, CoQSec, V, Sigma[i]));
      }
      ...
      for (unsigned int i = 0; i < n; i++)
      {
        sk.emplace (sk.end(), SecretKey (C, i, data1[i], data2[i], V, ZK, PK));
      }
